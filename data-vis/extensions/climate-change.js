    ...
    draw() {
        ...
        for (let i = 0; i < this.data.getRowCount(); i++) {
            ...
            if (previous != null
                ...
                const yearWidth = segmentWidth;
                const yearHeight = this.layout.bottomMargin-this.layout.topMargin;
                rect(this.mapYearToWidth(previous.year), this.layout.topMargin, yearWidth, yearHeight);
                ...
                if (dist(mouseX, mouseY, this.mapYearToWidth(previous.year), this.mapTemperatureToHeight(previous.temperature)) < 20){
                    stroke(255,0,0);
                    strokeWeight(10);
                    point(this.mapYearToWidth(previous.year), this.mapTemperatureToHeight(previous.temperature));
                    strokeWeight(1);
                    popupData.push({
                        label: previous.year,
                        data: `${previous.temperature.toFixed(2)}%`,
                        colour: "black"
                    });
                }
            }
